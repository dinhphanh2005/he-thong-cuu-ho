...
import { LayoutDashboard, Truck, Users, FileText, Settings, Search, Bell, LogOut, Wrench } from 'lucide-react';
        ...
        <div className="flex items-center justify-between p-2 group">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-gray-100 overflow-hidden shrink-0 border border-gray-200">
              <img
                src={`https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name || 'Admin')}&background=1f2937&color=fff`}
                alt="Avatar"
                className="w-full h-full object-cover"
              />
            </div>
            <div className="flex-1 min-w-0 text-left">
              <p className="text-[13px] font-black text-gray-900 truncate">{user?.name || 'Quản trị viên'}</p>
...
// ── Maintenance Badge (Admin biết đang bật, không bị block) ───────────────────
function MaintenanceBadge({ active }) {
  if (!active) return null;
  return (
    <div className="mx-4 mb-4 flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2">
      <Wrench size={14} className="text-amber-500 shrink-0" />
      <div>
        <p className="text-xs font-black text-amber-700">Chế độ bảo trì đang BẬT</p>
        <p className="text-[10px] text-amber-500">Người dùng không thể truy cập hệ thống</p>
      </div>
    </div>
  );
}
...
function LayoutInner() {
  const { isServerOffline, config } = useApp();
  ...
  return (
    <>
      ...
      >
        <div className="w-[280px] bg-white border-r border-gray-100 h-screen flex flex-col fixed left-0 top-0 z-20"
          style={{ paddingTop: isServerOffline ? '36px' : '0' }}>
          {/* Re-render Sidebar nội tuyến để dùng isMaintenanceOn */}
          <SidebarWithMaintenance isMaintenanceOn={isMaintenanceOn} />
        </div>
...
function SidebarWithMaintenance({ isMaintenanceOn }) {
  const navigate = useNavigate();
  const location = useLocation();
  const { config, user } = useApp();
  ...
  return (
    <>
      {/* Brand */}
      <div className="h-20 flex items-center shrink-0 px-8">
        <h1 className="text-gray-900 text-xl font-black tracking-tight font-sans truncate">
          {config?.systemName || 'Admin System'}
        </h1>
      </div>

      {/* Maintenance badge — chỉ admin thấy */}
      <MaintenanceBadge active={isMaintenanceOn} />
