...
export default function History({ navigation }: any) {
  ...
  // Refresh when screen comes back into focus (e.g. returning from Tracking)
  useFocusEffect(
    useCallback(() => {
      setPage(1);
  ...
  // Socket: update incident status in real-time without full re-fetch
  useEffect(() => {
    ...
    const handleIncidentUpdated = (payload: any) => {
      ...
      if (!updatedId || !payload.status) return;

      setIncidents(prev =>
        prev.map(inc =>
          inc._id === updatedId ? { ...inc, ...payload, _id: updatedId } : inc
        )
      );
    };

    socket.on('incident:updated', handleIncidentUpdated);
    return () => { socket.off('incident:updated', handleIncidentUpdated); };
  }, []);
  ...
  const getStatusDisplay = (item: any) => {
    ...
    if (item.status === 'PROCESSING') return { text: 'Đang xử lý', color: COLORS.primary, bg: '#E6F0FF' };
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    const isActive = !['COMPLETED', 'CANCELLED', 'HANDLED_BY_EXTERNAL'].includes(item.status);
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          if (isActive) navigation.navigate('Tracking', { incidentId: item._id });
        }}
      >
        ...
        <View style={styles.cardContent}>
          <View style={styles.cardHeader}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
              ...
              {isActive && <Ionicons name="chevron-forward" size={16} color={COLORS.gray} />}
            </View>
          </View>
          ...
          {isActive && (
            <View style={styles.trackHint}>
              ...
              <Text style={styles.trackHintText}>Nhấn để theo dõi vị trí đội cứu hộ</Text>
            </View>
          )}
        </View>
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  trackHint: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 8 },
  trackHintText: { fontSize: 12, color: COLORS.primary, fontWeight: '600' },
